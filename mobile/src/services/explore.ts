...
export const exploreService = {
   ...
   * SMART OVERLOAD: Supports 2 ways to call API.
   * - Method 1 (Day 1): getViewportChunks(minLat, maxLat, minLng, maxLng)
   * - Method 2 (Day 3): getViewportChunks(lat, lng, radius)
   */
 ...
 * const { success, data, error } = await exploreServiceLegacy.getStats();
 */
export const exploreServiceLegacy = {
  sendBatch: async (
    points: GpsPoint[],
  ): Promise<ApiResponse<BatchExploreResponse>> => {
    try {
      const data = await exploreService.sendBatch(points);
  ...
  getViewportChunks: async (
    ...
    try {
      const data = await exploreService.getViewportChunks(
        minLat,
        maxLat,
        minLng,
        maxLng,
  ...
  getStats: async (): Promise<ApiResponse<ExplorationStatsResponse>> => {
    try {
      const data = await exploreService.getStats();
      return { success: true, data };
    } catch (error: any) {
      return {
        success: false,
